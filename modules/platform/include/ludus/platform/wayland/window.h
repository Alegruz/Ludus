...
namespace ludus::platform::wayland
{
    ...
    public:
        ...
        bool IsClosed() const noexcept { return mIsClosed; }
        void SetClosed(bool value) noexcept { mIsClosed = value; }
    ...
    private:
        LUDUS_INLINE explicit WindowWayland(CreateInfo&& info) noexcept
            : WindowBase(info.BaseCreateInfo)
            , mSurface(std::move(info.Surface))
            , mXdgSurface(std::move(info.XdgSurface))
